...
def precision_equals(parallel_result: Iterable[float], serial_result: Iterable[float], precision: float) -> bool:
...
def relaxation_compile() -> None:
    subprocess.run(["gcc", "-fdiagnostics-color=always", "-g", "../serial.c", "-o", "../serial"])
    subprocess.run(["mpicc", "-fdiagnostics-color=always", "-g", "../parallel.c", "-o", "../parallel"])

def compute__results(test_data_filename: Path, serial_result_path: str, parallel_result_path: str) -> None:
    with open(serial_result_path, "w") as serial_result_file:
        subprocess.call(["../serial", str(test_data_filename)], stdout=serial_result_file)
    with open(parallel_result_path, "w") as parallel_result_file:
        subprocess.call(["mpirun", "-np", "4", "../parallel", str(test_data_filename)], stdout=parallel_result_file)
...
def relaxation_test(test_data_filename: Path, precision: float) -> bool:
    ...
    with open(serial_result_path, "r") as serial_result_file:
        serial_result = map(float, serial_result_file.read().replace("\n", "").strip().split(" "))
    with open(parallel_result_path, "r") as parallel_result_file:
        parallel_result = map(float, parallel_result_file.read().replace("\n", "").strip().split(" "))
