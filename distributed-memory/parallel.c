...
void flattenRows(float *flat, float **mat, size_t startRow, size_t endRow,
                 size_t rowSize) {
...
float calculateFlatNeighbourMean(float *matFlat, size_t centre, int denominator,
                                 size_t rowSize) {
    float neighbours[] = {matFlat[centre + rowSize], matFlat[centre + 1],
                          matFlat[centre - rowSize], matFlat[centre - 1]};
    return floatMean(neighbours, denominator);
}

void calculateProcessorChunkSizes(int *processorChunkSizes, size_t size,
                                  size_t nChunks) {
    // Chunk workload
...
void calculateProcessorChunkRows(int *processorChunkRows,
                                 int *processorChunkSizes, size_t nChunks) {
    // Calculates starting row for each chunk.
    processorChunkRows[0] = 0;
    for (size_t i = 1; i < nChunks; i++) {
        processorChunkRows[i] =
            processorChunkRows[i - 1] + processorChunkSizes[i - 1];
    }
}

void distributeChunkSizes(int *processorChunkRows, size_t nChunks,
                          size_t size) {
    ...
    for (size_t i = 0; i < nChunks; i++) {
        ...
        int endRow =
            i == nChunks - 1
                ? (int)size
                : (int)processorChunkRows
                      [i + 1]; // Set endRow to size if its the last chunk,
                               // otherwise the next chunk start row minus 1.
...
void distributeFlatChunks(int *processorChunkRows, size_t nChunks,
                          float *matFlat, size_t size) {
    ...
    for (size_t i = 0; i < nChunks; i++) {
        ...
        int n = (int)(endRow - startRow);
        int worker = (int)i + 1;
        MPI_Send(&matFlat[startRow * m], n * m, MPI_FLOAT, worker, 1,
                 MPI_COMM_WORLD);
    }
}

void collateFlatChunks(float *cpyFlat, size_t size, int *processorChunkRows,
                       size_t nChunks) {
    int m = (int)size;
    for (size_t i = 0; i < nChunks; i++) {
        int startRow = (int)processorChunkRows[i];
        ...
        int worker = (int)i + 1;
        float *start =
            &cpyFlat[m * (startRow + 1)]; // Receive from row 1 of the chunk
        int nValues =
            m * ((int)n - 2); // End at the penultimate row of the chunk
        MPI_Recv(start, nValues, MPI_FLOAT, worker, 2, MPI_COMM_WORLD, 0);
    }
}

void updateFlatEdgeRows(float *matFlat, float *cpyFlat, size_t nChunks,
                        size_t rowSize, int *processorChunkRows) {
    ...
    for (size_t i = 1; i < nChunks; i++) {
        ...
        cpyFlat[flatLeadingEdgeRow + rowSize - 1] =
            matFlat[flatLeadingEdgeRow + rowSize - 1];
        cpyFlat[flatTrailingEdgeRow] = matFlat[flatTrailingEdgeRow];
        cpyFlat[flatTrailingEdgeRow + rowSize - 1] =
            matFlat[flatTrailingEdgeRow + rowSize - 1];
        // Compute values
        for (size_t j = 1; j < rowSize - 1; j++) {
            ...
            cpyFlat[flatLeadingEdgeRow + j] = calculateFlatNeighbourMean(
                matFlat, flatLeadingCentre, 4, rowSize);
            cpyFlat[flatTrailingEdgeRow + j] = calculateFlatNeighbourMean(
                matFlat, flatTrailingCentre, 4, rowSize);
        }
    }
}
    ...
    for (size_t i = 1; i < size - 1; i++) {
        for (size_t j = 1; j < size - 1; j++) {
            ...
            if (fabs(matFlat[index] - cpyFlat[index]) > PRECISION) {
                return false; // At least one element in the matrix is outside
                              // PRECISION
            }
        }
    }
...
void terminateSlaves(int *processorChunkSizes, size_t nChunks, size_t size) {
    ...
    for (size_t i = 0; i < nChunks; i++) {
        ...
        int worker = (int)i + 1;
        MPI_Send(terminal, rows * (int)size, MPI_FLOAT, worker, 1,
                 MPI_COMM_WORLD);
    }
}
...
void relaxationMaster(float **mat, size_t size, size_t nProcessors,
                      bool logging) {
    bool stop = false;
    ...
    memcpy(cpyFlat, matFlat,
           size * size * sizeof(float)); // Create copy of mat.

    // Configure chunks
    calculateProcessorChunkSizes(processorChunkSizes, size, nChunks);
    calculateProcessorChunkRows(processorChunkRows, processorChunkSizes,
                                nChunks);
    ...
    while (!stop) {
        ...
        distributeFlatChunks(processorChunkRows, nChunks, matFlat,
                             size); // Distribute to worker cores
        updateFlatEdgeRows(matFlat, cpyFlat, nChunks, size,
                           processorChunkRows); // Update edge rows in chunks
                                                // that don't have 4 neighbours
        collateFlatChunks(
            cpyFlat, size, processorChunkRows,
            nChunks); // Receive completed computations from worker cores
        stop = flatPrecisionStopCheck(matFlat, cpyFlat, size);
        arraySwap(&matFlat, &cpyFlat);
        if (stop) {
            terminateSlaves(processorChunkSizes, nChunks,
                            size); // Send termination signal to slaves.
...
void relaxationSlave() {
    ...
    while (!stop) {
        ...
        if (terminated(matFlat)) { // Check if termination message
            ...
            processChunk(matFlat, cpyFlat, n,
                         m); // Perform relaxation calculations
            MPI_Send(&cpyFlat[m], (int)m * ((int)n - 2), MPI_FLOAT, 0, 2,
                     MPI_COMM_WORLD);
        }
    }
}

void relaxation(char *dataFilePath, size_t nProcessors, int mpiRank,
                bool logging) {
    if (mpiRank == 0) { // master
        size_t size;
